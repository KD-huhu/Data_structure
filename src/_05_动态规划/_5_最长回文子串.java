...
public class _5_最长回文子串 {
	/**
	 * 动态规划： dp是大小为n*n的二维数组，dp[i][j]表示s[i][j]是否为回文串
	 * 1. s[i][j]的长度(j-i+1)<=2, s[i]=s[j], s[i][j]是回文串
	 * 		dp[i][j] = (s[i] == s[j])
	 * 2. s[i][j]的长度(j-i+1)>2, 如果s[i+1][j-1]是回文串, s[i]=s[j], s[i][j]是回文串
    ...
    /**
	 * 扩展中心法2
	 * 使用两个指针优化，先找出第一个和当前指针所指向的字符不想通过的字符
	 * 通过该字符和当前-1开始扩展
	 * 之后跳过所有的相同字符，直接扫描之后不行同的字符
	 * 优化是因为连续的相同的字符一定是回文串的中心部分
	 */
